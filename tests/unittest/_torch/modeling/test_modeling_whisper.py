...
def _tiny_whisper_model_config() -> "ModelConfig":
    ...
    return ModelConfig(
        pretrained_config=config, attn_backend="TRTLLM", is_generation=True, is_encoder_decoder=True
    )
...
def _decoder_forward_logits(
    ...
    attn_metadata = metadata_cls(
        ...
        # No-cache context metadata: q and kv spans are the same prompt length.
    ...
    # Decoder cross-attention needs its own metadata built from the encoder
    # context length, matching what WhisperModel.forward forwards to the decoder.
...
def test_whisper_for_conditional_generation_construction_and_forward():
    ...
    # Whisper ties proj_out (lm_head) to the decoder token embedding.
    ...
    # Distinct decoder token ids over the same encoder context change the logits.
    assert not torch.allclose(logits_a, logits_b)
    # Distinct encoder context with identical decoder token ids changes the
    # logits, proving cross-attention consumes the encoder hidden states.

...
from tensorrt_llm.bindings import DataType, internal
...
def _make_kv_manager(config: BartConfig, num_layers: int, is_cross: bool):
    ...
    cache_type = (
        internal.batch_manager.CacheType.CROSS
        if is_cross
        else internal.batch_manager.CacheType.SELF
    )
...
def test_bart_for_conditional_generation_construction_and_forward():
    ...
    enc_len = 4
    encoder_hidden_a = torch.linspace(
        -1.0,
        1.0,
        steps=enc_len * config.d_model,
        dtype=torch.float16,
    ).reshape(enc_len, config.d_model)
    encoder_hidden_b = -encoder_hidden_a

    self_kv, cross_kv, attn_md, device, dec_len = _make_decoder_run(
        config,
        decoder_input_ids,
    )
    ...
    assert not torch.allclose(logits_a, logits_b, rtol=1e-5, atol=1e-5)

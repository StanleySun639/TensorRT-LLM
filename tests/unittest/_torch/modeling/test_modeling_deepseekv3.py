...
def _no_decoder_reference(model, input_ids):
    ...
    # model's own weight tensors and its real head linear op.
    with torch.inference_mode():
...
class TestDeepseekV32ForCausalLM:
    def test_construct_registered_v32_and_forward(self):
        ...
        # fixed-result forward cannot satisfy this jointly with the shifted
        # input inequality below.
        with torch.inference_mode():
        ...
        # itself input-sensitive so the not-allclose comparison below is
        # discriminating for the real decoder/MLA path.

...
def _make_helix_mapping(
    tp_size=4, cp_size=2, rank=0, enable_attention_dp=False, moe_tp_size=None, moe_ep_size=None
):
    kwargs = dict(
    ...
    if moe_tp_size is not None:
        kwargs["moe_tp_size"] = moe_tp_size
    if moe_ep_size is not None:
        kwargs["moe_ep_size"] = moe_ep_size
...
def test_setup_helix_mappings_precondition_validation():
    ...
    mapping = _make_helix_mapping(tp_size=4, cp_size=2, moe_tp_size=2, moe_ep_size=2)
    model_config = _make_model_config(mapping)
    ...
    # num_experts not divisible by moe_ep raises
    mapping = _make_helix_mapping(tp_size=4, cp_size=2)
    model_config = _make_model_config(mapping)
...
def test_setup_helix_mappings_dual_mapping_swap_restore():
    ...
    # model_config.mapping is the repurposed mapping

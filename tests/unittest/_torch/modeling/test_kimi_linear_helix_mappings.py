...
def _make_helix_mapping(tp_size=4, cp_size=2, rank=0, enable_attention_dp=False):
    """Build a Mapping with cp_size>1. Does NOT pass moe kwargs to avoid the
    CP ULYSSES guard in the Mapping constructor."""
    mapping = Mapping(
...
def _set_moe_attrs(mapping, moe_tp_size, moe_ep_size):
    """Manually set moe attributes post-construction to bypass the CP ULYSSES
    guard that rejects moe_tp/ep kwargs with cp_size>1."""
...
def test_setup_helix_mappings_dual_mapping_swap_restore():
    ...
    def _capture_run_concurrently(fn_list, max_workers=None):
        for fn in fn_list:
            fn()

    # Shape arithmetic: full_size = shard_size * repurposed_tp so that
    # src.shape[1] % param.shape[1] == 0 triggers the down_proj row-shard
    # branch with shard_count == repurposed_tp.
    shard_size = 4
    ...
    # Under helix, model_tp_rank comes from _repurposed_tp_mapping.tp_rank
    ...
    # model_config.mapping.tp_rank == 0, so slice starts at 0
